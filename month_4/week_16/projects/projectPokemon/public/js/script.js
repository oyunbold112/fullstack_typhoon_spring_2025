    ...
    for (let i = 0; i < pokemons.length; i++) {
      ...
      fetch(pokemonDetailUrl)
        ...
        .then((data) => {
          ...
          pokemonDetailContainer.addEventListener("click", () =>
            showModal(data, backgroundColorByType)
          );
          // types of pokemon
...
function searchData() {
  let searchResult = globalData
    .slice()
    .filter((e) => e.name.includes(searchInput.value));
...
function showModal(data, bgcolor) {
  ...
  console.log("ene mondoo");
  console.log(typeList[0].type.name);
  for (let i = 0; i < typesElement.children.length; i++) {
    typesElement.children[i].remove();
  }
  for (let i = 0; i < typeList.length; i++) {
    const typeElement = document.createElement("p");
    typeElement.classList.add("type-element");
